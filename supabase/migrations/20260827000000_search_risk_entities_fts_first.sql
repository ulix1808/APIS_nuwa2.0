-- Fix catalog search timeouts at ~780k chunks.
-- Previous WHERE used word_similarity(...) OR fts @@ q, which forced a near-full
-- scan (~29s for "pemex") and exceeded the BFF 25s client abort.
-- Strategy: filter with GIN FTS indexes first; score with word_similarity only on candidates.
...
BEGIN
  ...
  IF r IS NOT NULL THEN
    ...
    r_has_fts := q_ts_rfc IS NOT NULL AND coalesce(numnode(q_ts_rfc), 0) > 0;
  ELSE
    q_ts_rfc := NULL;
  END IF;
  ...
  SELECT
    ...
    (
      ...
      CASE
        ...
        WHEN r_has_fts AND c.fts @@ q_ts_rfc
    ...
    CASE
      ...
      WHEN q_has_fts AND c.fts @@ q_ts THEN
        ts_headline(
          'simple',
          c.chunk_text,
          q_ts,
      ...
      WHEN r_has_fts AND c.fts @@ q_ts_rfc THEN
        ts_headline(
          'simple',
          c.chunk_text,
          q_ts_rfc,
  ...
  WHERE
    ...
    AND (
      (
        q <> ''
        AND (
          -- Indexed FTS path (primary). Avoid word_similarity in WHERE — full scan at 780k+.
          (
            q_has_fts
            AND (
              (c.chunk_text_normalized IS NOT NULL AND trim(c.chunk_text_normalized) <> '' AND c.fts_normalized @@ q_ts)
          ...
          -- Rare fallback when websearch_to_tsquery yields nothing (symbols-only query).
          OR (
            NOT q_has_fts
            AND word_similarity(
              q,
              coalesce(nullif(trim(c.chunk_text_normalized), ''), c.chunk_text)
            ) >= p_word_similarity_threshold
          )
        )
      )
      OR (
...
COMMENT ON FUNCTION public.search_risk_entities IS
  'FTS-first search on risk_entity_chunks (GIN). word_similarity used for scoring / rare non-FTS fallback — not in the main WHERE filter.';
