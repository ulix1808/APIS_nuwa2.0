-- Tighten CJF FTS-first search: avoid loose trigram OR (pulled ~28k candidates),
-- and truncate tema/sintesis so Lambda/API payloads stay small.
...
BEGIN
  ...
  -- Prefer a tight GIN path. Do not OR with loose % (that expanded to tens of thousands of rows).
  ...
  candidates AS (
    SELECT
      ...
      d.url_sise,
      -- Truncate heavy text fields (full text is available via documento_id / url_sise)
...
COMMENT ON FUNCTION public.search_cjf_mentions(TEXT, INT, TEXT) IS
  'CJF legal name search — FTS GIN candidates + token strictness; tema/sintesis truncated to 600 chars';
